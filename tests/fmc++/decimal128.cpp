...
TEST(decimal128, exp63_check) {
  ...
  fmc_decimal128_from_str(&table[2], "85070591730234615865843651857942052864", &err);
  fmc_decimal128_from_str(
      &table[3], "784637716923335095479473677900958302012794430558004314112", &err);
  fmc_decimal128_from_str(
      &table[4], "72370055773322622139731865630429942408293740416025352524"
                 "66099000494570602496", &err);
  fmc_decimal128_from_str(
      &table[5], "66749594872528440074844428317798503581334516323645399060"
                 "845050244444366430645017188217565216768", &err);
  fmc_decimal128_from_str(
      &table[6],
      "6156563468186637376918600015647439657043709261010226041866920844413394"
      "02679643915803347910232576806887603562348544", &err);
  fmc_decimal128_from_str(
      &table[7],
      "5678427533559428832416592249125035424637823130369672345949142181098744"
      "438385921275985867583701277855943457200048954515105739075223552", &err);
  fmc_decimal128_from_str(
      &table[8], "52374249726338269920211035149241586435466272736689036631"
                 "73266188953814074247479287813232147721446651441418694604"
                 "0961136147476104734166288853256441430016", &err);
  fmc_decimal128_from_str(
      &table[9],
      ...
      "7863584183385093587122601852928", &err);
  fmc_decimal128_from_str(
      &table[10],
      ...
      "68614570041224793214838549179946240315306828365824", &err);
  fmc_decimal128_from_str(
      &table[11],
      ...
      "686052895717819140275184930690973423372373108471271228681978529185792", &err);
  fmc_decimal128_from_str(
      &table[12],
  ...
  fmc_decimal128_from_str(
      ...
      "4779776678907072458937421726941708288", &err);
  fmc_decimal128_from_str(
      &table[14],
      ...
      "92245363054977512313161523248215761503691988438775496704", &err);
  fmc_decimal128_from_str(
      &table[15],
  ...
  fmc_decimal128_from_str(
      ...
      "3812816523290938580750782913463467636686848", &err);
...
TEST(decimal128, exp63_checkdec) {
  ...
  fmc_decimal128_from_str(&table[1], "9223372036854775808", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[2], "8507059173023461586584365185794205E+4", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[3], "7846377169233350954794736779009583E+23", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[4], "7237005577332262213973186563042994E+42", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[5], "6674959487252844007484442831779850E+61", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[6], "615656346818663737691860001564744E+81", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[7], "5678427533559428832416592249125035E+99", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[8], "5237424972633826992021103514924159E+118", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[9], "4830671903771572930869189863664984E+137", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[10], "4455508415646675018204269146191691E+156", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[11], "4109481173084666802532023346000101E+175", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[12], "3790327373781027673703563204254157E+194", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[13], "3495959950985713037648777335653667E+213", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[14], "3224453925388581825880980132547098E+232", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[15], "2974033816955566125596124996299801E+251", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[16], "2743062034396844341627968125593605E+270", &err);
  EXPECT_EQ(err, nullptr);
  fmc_decimal128_from_str(&table[17], "2530028166341382729406191833986466E+289", &err);
  EXPECT_EQ(err, nullptr);
