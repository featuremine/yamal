...
TEST(channel, sequential) {
  ...
  EXPECT_TRUE(fmc_fvalid(fd));
  {
    ...
    for (unsigned i = 1; i < test_size; ++i) {
      ...
      auto *msg =
          (test_msg *)ytp_channel_reserve(yamal, sizeof(test_msg), &error);
      ASSERT_NE(msg, nullptr);
      msg->index = i;
      ASSERT_NE(ytp_channel_commit(yamal, peer, channel, msg, &error), nullptr);
    }
...

  {
    ...
    for (; !ytp_yamal_term(iter); iter = ytp_yamal_next(yamal, iter, &error)) {
      ...
      size_t sz;
      test_msg *msg;
      ytp_channel_read(yamal, iter, &peer, &channel, &sz, (const char **)&msg,
                       &error);
      ASSERT_EQ(error, nullptr);
      last_idx = msg->index;
...
TEST(channel, peer_name) {
...

  {
    ...
    size_t sz;
    const char *name;
    ytp_channel_read(yamal, iter, &peer, &channel, &sz, &name, &error);
    ASSERT_EQ(error, nullptr);
    ASSERT_EQ(peer, 0);
    ASSERT_EQ(sz, 4);
