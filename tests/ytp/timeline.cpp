...
struct callback_helper {
  ...
  template <typename F> std::pair<ytp_timeline_sub_cb_t, void *> subcb(F f) {
    auto *cl =
        callbacks
            .emplace_back(std::make_unique<closure_wrapper<F>>(std::move(f)))
            .get();
...
TEST(timeline, data_simple_subscription_1) {
  ...
  ASSERT_EQ(error, nullptr);
  auto channel1 =
      ytp_control_ch_decl(ctrl, producer1, 0, 13, "main/channel1", &error);
  ASSERT_EQ(error, nullptr);
...
TEST(timeline, data_multiple_channel_1) {
  ...
  ASSERT_EQ(error, nullptr);
  auto channel2 =
      ytp_control_ch_decl(ctrl, producer1, 0, 18, "secondary/channel2", &error);
  ASSERT_EQ(error, nullptr);
  ASSERT_NE(consumer1, 0);
  ASSERT_NE(producer1, 0);
  ASSERT_NE(channel1, 0);
...
TEST(timeline, data_multiple_producers_1) {
  ...
  ASSERT_EQ(error, nullptr);
  channel1 =
      ytp_control_ch_decl(ctrl, producer2, 0, 13, "main/channel1", &error);
  ASSERT_EQ(error, nullptr);
  ASSERT_NE(consumer1, 0);
  ASSERT_NE(producer1, 0);
...
TEST(timeline, data_subscription_first_1) {
  ...
  ASSERT_EQ(error, nullptr);
  auto channel3 =
      ytp_control_ch_decl(ctrl, producer1, 0, 13, "main/channel3", &error);
  ASSERT_EQ(error, nullptr);
...
TEST(timeline, idempotence_simple_1) {
  ...
  auto channel2 =
      ytp_control_ch_decl(ctrl, producer2, 1000, 13, "main/channel2", &error);
  auto channel2_2 =
      ytp_control_ch_decl(ctrl, producer2, 1000, 13, "main/channel2", &error);
