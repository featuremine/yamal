  ...
  template <typename F> std::pair<ytp_cursor_data_cb_t, void *> datacb(F f) {
    auto *cl =
        callbacks
            ...
            .get();
    return {&static_cb<F, size_t, uint64_t, ytp_stream_t, size_t, const char *>, cl};
  }
  ...
  template <typename F> std::pair<ytp_cursor_ann_cb_t, void *> anncb(F f) {
    auto *cl =
        callbacks
            .emplace_back(std::make_unique<closure_wrapper<F>>(std::move(f)))
            .get();
    return {&static_cb<F, ytp_stream_t, size_t, size_t, const char *, size_t, const char *, size_t, const char *>, cl};
  }
...
TEST(stream, main_test_1) {
  ...
  auto stream11 = ytp_anns_stream(anns, 5, "peer1", 3, "ch1", 9, "encoding1", &error);
  ASSERT_EQ(error, nullptr);

  auto stream12 = ytp_anns_stream(anns, 5, "peer1", 3, "ch2", 9, "encoding2", &error);
  ASSERT_EQ(error, nullptr);

  auto stream21 = ytp_anns_stream(anns, 5, "peer2", 3, "ch1", 9, "encoding3", &error);
  ASSERT_EQ(error, nullptr);
  ...
  auto stream22 = ytp_anns_stream(anns, 5, "peer2", 3, "ch2", 9, "encoding4", &error);
  ASSERT_EQ(error, nullptr);
  ...
  auto stream12_redef = ytp_anns_stream(anns, 5, "peer1", 3, "ch2", 9, "encoding2", &error);
  ASSERT_EQ(error, nullptr);
  ...
  struct MsgIdx : std::tuple<ytp_stream_t, uint64_t, std::string_view> { using tuple::tuple; };
  ...
  auto publish = [&](const Msg &msg) {
    return std::visit(fmc::overloaded{
        [&] (const MsgAnn &msg) {
          auto &[p, c, e] = msg;
          auto it = ytp_stream_write_ann(yamal, p.size(), p.data(), c.size(), c.data(), e.size(), e.data(), &error);
        ...
        [&] (const MsgIdx &msg) {
          auto &[stream, offdata, payload] = (std::tuple<ytp_stream_t, uint64_t, std::string_view> &)msg;
          auto it = ytp_stream_write_idx(yamal, stream, offdata, payload.size(), payload.data(), &error);
          EXPECT_EQ(error, nullptr);
          auto off = ytp_yamal_tell(yamal, it, &error);
          EXPECT_EQ(error, nullptr);
          return off;
...

  {
    ...
    {
      auto cb = helper.anncb([&](ytp_stream_t stream, size_t seqno, size_t peer_sz, const char *peer_name, size_t ch_sz, const char *ch_name, size_t encoding_sz, const char *encoding_data) {
        ...
        if (peername != "peer1" || chname != "ch1") {
          auto cb = helper.datacb(
              [&](size_t seqno, uint64_t msgtime, ytp_stream_t stream, size_t sz, const char *data) {
    ...
    EXPECT_EQ(readoneraw(YTP_STREAM_LIST_DATA), buildmsg(uint64_t{5005}, stream11, "0000"));
    EXPECT_EQ(readoneraw(YTP_STREAM_LIST_DATA), buildmsg(uint64_t{5006}, stream21, "0001"));
    EXPECT_EQ(readoneraw(YTP_STREAM_LIST_DATA), buildmsg(uint64_t{5007}, stream12, "0002"));
    EXPECT_EQ(readoneraw(YTP_STREAM_LIST_DATA), buildmsg(uint64_t{5009}, stream22, "0003"));
    EXPECT_EQ(readoneraw(YTP_STREAM_LIST_DATA), buildmsg(uint64_t{5009}, stream11, "0004"));
    EXPECT_EQ(readoneraw(YTP_STREAM_LIST_DATA), buildmsg(uint64_t{5009}, stream12, "0005"));
    EXPECT_EQ(readoneraw(YTP_STREAM_LIST_DATA), buildmsg(uint64_t{5009}, stream21, "0006"));
