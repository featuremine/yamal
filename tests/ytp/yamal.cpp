...
static_assert(sizeof(ytp_mmnode) == YTP_MMNODE_HEADER_SIZE);

static_assert(sizeof(ytp_hdr) == YTP_YAMAL_HEADER_SIZE);
...
static void seektell(bool enable_thread, bool close_file) {
  fmc_error_t *error;
  auto fd = fmc_ftemp(&error);
  ASSERT_EQ(error, nullptr);
  ASSERT_TRUE(fmc_fvalid(fd));
  auto *yamal = ytp_yamal_new_3(fd, enable_thread, YTP_CLOSABLE, &error);
  ASSERT_EQ(error, nullptr);
  ASSERT_NE(yamal, nullptr);
  auto it = ytp_yamal_begin(yamal, 0, &error);
  ASSERT_EQ(error, nullptr);
  ...
  error = (fmc_error_t *)1;
  EXPECT_EQ(ytp_yamal_tell(yamal, ytp_yamal_begin(yamal, 0, &error), &error),
            16);
  ASSERT_EQ(error, nullptr);
  EXPECT_EQ(ytp_yamal_tell(yamal, zeroit, &error), 16);
  ASSERT_EQ(error, nullptr);
  EXPECT_EQ(ytp_yamal_tell(yamal, oneit, &error), 536);
  ASSERT_EQ(error, nullptr);
  EXPECT_EQ(ytp_yamal_tell(yamal, ytp_yamal_end(yamal, 0, &error), &error),
            584);
  ASSERT_EQ(error, nullptr);

  error = (fmc_error_t *)1;
  EXPECT_EQ(ytp_yamal_seek(yamal, 16, &error),
            ytp_yamal_begin(yamal, 0, &error));
  ASSERT_EQ(error, nullptr);
  EXPECT_EQ(ytp_yamal_seek(yamal, 536, &error), oneit);
  ASSERT_EQ(error, nullptr);
  EXPECT_EQ(ytp_yamal_seek(yamal, 584, &error),
...
TEST(yamal, closable_empty_list) {
  ...
  ASSERT_FALSE(ytp_yamal_closed(yamal, 1, &error));
  ASSERT_FALSE(ytp_yamal_closed(yamal, 2, &error));
...
TEST(yamal, closable_write) {
  ...
  error = (fmc_error_t *)1;
  ASSERT_EQ(ytp_yamal_commit(yamal, msg2, 1, &error), nullptr);
  ASSERT_NE(error, nullptr);
  ASSERT_EQ(error->code, FMC_ERROR_CLOSED);

  error = (fmc_error_t *)1;
  ASSERT_EQ(ytp_yamal_commit(yamal, msg2, 2, &error), nullptr);
  ASSERT_NE(error, nullptr);
  ASSERT_EQ(error->code, FMC_ERROR_CLOSED);

  error = (fmc_error_t *)1;
  ASSERT_EQ(ytp_yamal_commit(yamal, msg2, 3, &error), nullptr);
  ASSERT_NE(error, nullptr);
  ASSERT_EQ(error->code, FMC_ERROR_CLOSED);
  ...
  error = (fmc_error_t *)1;
  ASSERT_EQ(ytp_yamal_commit(yamal, msg3, 1, &error), nullptr);
  ASSERT_NE(error, nullptr);
  ASSERT_EQ(error->code, FMC_ERROR_CLOSED);

  error = (fmc_error_t *)1;
  ASSERT_EQ(ytp_yamal_commit(yamal, msg3, 2, &error), nullptr);
  ASSERT_NE(error, nullptr);
  ASSERT_EQ(error->code, FMC_ERROR_CLOSED);

  error = (fmc_error_t *)1;
  ASSERT_EQ(ytp_yamal_commit(yamal, msg3, 3, &error), nullptr);
  ASSERT_NE(error, nullptr);
  ASSERT_EQ(error->code, FMC_ERROR_CLOSED);
