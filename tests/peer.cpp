...
static void sequential(bool enable_thread) {
  ...
  make_temp_file(&f, pool);
  {
    ...
    for (unsigned i = 1; i < test_size; ++i) {
      ...
      test_msg *msg = NULL;
      rv = ytp_peer_reserve(yamal, (char **)&msg, sizeof(test_msg));
      ASSERT_EQ(rv, YTP_STATUS_OK);
      ASSERT_NE(msg, nullptr);
      msg->index = i;
      rv = ytp_peer_commit(yamal, &it, peer, msg);
      ASSERT_EQ(rv, YTP_STATUS_OK);
    }
...

  {
    ...
    for (; !ytp_yamal_term(it); rv = ytp_yamal_next(yamal, &it, it)) {
      ...
      apr_size_t sz;
      test_msg *data;
      rv = ytp_peer_read(yamal, it, &peer, &sz, (const char **)&data);
      ASSERT_EQ(rv, YTP_STATUS_OK);
...
static void name(bool enable_thread) {
  apr_pool_t *pool = NULL;
  apr_file_t *f = NULL;
  test_init(&pool);
  make_temp_file(&f, pool);
  ...
  }
  {
    ...
    apr_size_t sz;
    const char *name;
    rv = ytp_peer_read(yamal, it, &peer, &sz, (const char **)&name);
    ASSERT_EQ(rv, YTP_STATUS_OK);
    ASSERT_EQ(peer, 0);
    ASSERT_EQ(sz, 4);
