...
static void parse_value(struct ini_sect *ini, struct fmc_cfg_type *spec,
  ...
  case FMC_CFG_STR: {
    // If we start with a quote it must end with a quote, no characters afterwards
    // If it does not start with a quote we use the whole thing
    bool quoted = **str == '"';
    ++*str;
    char *endptr = NULL;
    if (quoted) {
...
struct fmc_cfg_sect_item *fmc_cfg_sect_item_get(struct fmc_cfg_sect_item *cfg,
                                                const char *key) {
  struct fmc_cfg_sect_item *item = NULL;
