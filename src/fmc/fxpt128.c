...
static void fmc_fxpt128__umul(struct fmc_fxpt128_t *dst, const struct fmc_fxpt128_t *a, const struct fmc_fxpt128_t *b)
{
   ...
   p0.lo = p0.hi; p0.hi = 0; //fmc_fxpt128_shr(&p0, &p0, 64);
   ...
   p3.hi = p3.lo; p3.lo = 0; //fmc_fxpt128_shl(&p3, &p3, 64);
...
refine1:
...

   {
      ...
      t2.hi = t2.lo; t2.lo = 0;  //fmc_fxpt128_shl(&t2, &t2, 64);
...
void fmc_fxpt128_from_str(struct fmc_fxpt128_t *dst, const char *s, fmc_error_t **err)
{
    fmc_error_clear(err);
    char *endptr;
    fmc_fxpt128_from_string(dst, s, &endptr);
    if (s + strlen(s) > endptr)
        fmc_error_set(err, "cannot parse %s as fxpt128", s);
}
