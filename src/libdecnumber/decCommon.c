...
#if QUAD
const uInt DECCOMBMSD[64] = {0, 1, 2, 3, 4, 5, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7,
...
/* [exponent unchanged] */
    ...
    if (num->exponent > DECEMAX - (DECPMAX - 1)) { /* is edge case */
      ...
      if (*ulsd == 0 && ulsd == umsd) {                    /* have zero */
        ...
        case DEC_ROUND_CEILING: {
          if (num->sign)
            needmax = 1; /* Infinity iff non-negative */
          break;
        } /* r-c */
        case DEC_ROUND_FLOOR: {
          if (!num->sign)
            needmax = 1; /* Infinity iff negative */
          break;
        } /* r-f */
        default:
  ...
  if (!NUMISSPECIAL(num)) { /* is still finite */
    ...
    encode |=
        (uexp << (32 - 6 - DECECONL)) & 0x03ffffff; /* exponent continuation */
  } else
...
/* expand coefficient and sign to BCDAR */
  ...
  op--; /* -> sign byte */
  if (*op == DECPMINUS || *op == DECPMINUSALT)
    sig = DECFLOAT_Sign;
...
/* expand coefficient and sign to BCDAR */
  ...
  for (ip = packed; ip < packed + ((DECPMAX + 2) / 2); ip++) {
    *op = *ip >> 4;
    ...
    op++;
    *op = (uByte)(*ip & 0x0f); /* [final nibble is sign] */
    if (*op > 9 && ip < packed + ((DECPMAX + 2) / 2) - 1)
      return NULL;
    op++;
  }
  op--; /* -> sign byte */
  if (*op <= 9)
    return NULL; /* bad sign */
  if (*op == DECPMINUS || *op == DECPMINUSALT)
    sig = DECFLOAT_Sign;

#if !SINGLE
  if (bcdar[0] != 0)
    return NULL; /* bad pad nibble */
#endif
  ...
  if (EXPISNAN(exp)) { /* a NaN */
    ...
    for (i = 0; i < DECPMAX; i++) {
      if (bcdar[i + 1] != 0)
        return NULL; /* should be all zeros */
    }
  }      /* infinity */
  else { /* finite */
    /* check the exponent is in range */
    if (exp > DECEMAX - DECPMAX + 1)
      return NULL;
    if (exp < DECEMIN - DECPMAX + 1)
      return NULL;
  }
  return decFloatFromBCD(df, exp, bcdar + 1, sig);
} /* decFloatFromPacked */
      ...
      if (decBiStr(c, "infinity", "INFINITY") || decBiStr(c, "inf", "INF"))
        ...
        if (*cfirst != '\0') { /* not empty or all-0, payload */
          ...
          for (c = cfirst;; c++, ub++) {
            ...
            if (c - cfirst == DECPMAX - 1)
              break;                 /* too many digits */
            *ub = (uByte)(*c - '0'); /* good bcd8 */
          }
          if (*c != '\0')
            break;          /* not all digits, or too many */
...
void decFloatShow(const decFloat *df, const char *tag) {
  char
      hexbuf[DECBYTES * 2 + DECBYTES / 4 + 1]; /* NB blank after every fourth */
...
#define dpd2char(dpdin)                                                        \
  ...
  if (pre > 0) { /* ddd.ddd (plain), perhaps with E */
    ...
    t = cstart + ROUNDDOWN4(c - cstart) +
        pre; /* preferred first target point */
    /* backoff if too far to the right */
    if (t > string + DECSTRING - 5)
      t = string + DECSTRING - 5; /* adjust to fit */
    /* now shift the entire coefficient to the right, being careful not */
    /* to access to the left of string [cannot use memcpy] */
    for (s = t - pre; s >= string; s -= 4, t -= 4)
      UBFROMUI(t, UBTOUI(s));
...
/* Three-character exponents are easy; 4-character a little trickier */
    ...
    if (e < 1000) {                         /* 3 (or fewer) digits case */
      ...
      *c++ = (char)('0' + (char)thou);    /* the thousands digit */
      ...
      UBFROMUI(c,
               UBTOUI(u) | CHARMASK); /* copy fixed 3+1 characters [is safe] */
...
/* Three-character exponents are easy; 4-character a little trickier */
      ...
      if (e < 1000) {                       /* 3 (or fewer) digits case */
        ...
        *c++ = (char)('0' + (char)thou);    /* the thousands digit */
        u = &BIN2BCD8[rem * 4];             /* -> 3 digits + length byte */
        UBFROMUI(c, UBTOUI(u) |
                        CHARMASK); /* copy fixed 3+1 characters [is safe] */
        c += 3;                    /* bump pointer, always 3 digits */
      }
#endif
    }
...
decFloatWider *decFloatToWider(const decFloat *source, decFloatWider *wider) {
  ...
  if (DFISSPECIAL(source)) {
    ...
    uInt exp =
        GETEXPUN(source) + DECWBIAS;      /* get unbiased exponent and rebias */
    uInt code = (exp >> DECWECONL) << 29; /* set two bits of exp [msd=0] */
    code |= (exp << (32 - 6 - DECWECONL)) &
