...
static decFloat *decDivide(decFloat *, const decFloat *, const decFloat *,
                           decContext *, uInt);
...
static decFloat *decToIntegral(decFloat *, const decFloat *, decContext *,
                               enum rounding, Flag);
static uInt decToInt32(const decFloat *, decContext *, enum rounding, Flag,
                       Flag);
...
static decFloat *decCanonical(decFloat *result, const decFloat *df) {
  ...
  Int n;                     /* counter (down) */
  if (df != result)
    *result = *df; /* effect copy if needed */
  if (DFISSPECIAL(result)) {
    if (DFISINF(result))
      return decInfinity(result, df); /* clean Infinity */
    /* is a NaN */
    DFWORD(result, 0) &= ~ECONNANMASK; /* clear ECON except selector */
    if (DFISCCZERO(df))
      return result; /* coefficient continuation is 0 */
  ...
  { /* declare block */
    ...
    if (CANONDPDOFF(sourhi, 8) && CANONDPDTWO(sourhi, sourlo, 30) &&
        CANONDPDOFF(sourlo, 20) && CANONDPDOFF(sourlo, 10) &&
        CANONDPDOFF(sourlo, 0))
      return result;
#elif QUAD
    uInt sourhi = DFWORD(df, 0);
    ...
    if (CANONDPDOFF(sourhi, 4) && CANONDPDTWO(sourhi, sourmh, 26) &&
        CANONDPDOFF(sourmh, 16) && CANONDPDOFF(sourmh, 6) &&
        CANONDPDTWO(sourmh, sourml, 28) && CANONDPDOFF(sourml, 18) &&
        CANONDPDOFF(sourml, 8) && CANONDPDTWO(sourml, sourlo, 30) &&
        CANONDPDOFF(sourlo, 20) && CANONDPDOFF(sourlo, 10) &&
        CANONDPDOFF(sourlo, 0))
      return result;
#endif
...
static decFloat *decDivide(decFloat *result, const decFloat *dfl,
  ...
  if (DFISSPECIAL(dfl) || DFISSPECIAL(dfr)) { /* either is special? */
    ...
    if (DFISINF(dfl)) {
      if (DFISINF(dfr))
        return decInvalid(result, set); /* Two infinities bad */
      if (op & (REMAINDER | REMNEAR))
        return decInvalid(result, set); /* as is rem */
...
#error Unexpected Quad DIVOPLEN
  ...
  for (msud = div + DIVOPLEN - 1; *msud == 0;)
    msud--;
...
#if DECTRACE
  {
    Int i;
    printf("----- div=");
    for (i = divunits - 1; i >= 0; i--)
      printf("%09ld ", (LI)div[i]);
    printf("\n");
  }
#endif
...
#define DIVSHIFTB 32
      ...
      if (carry)
        *ua -= carry; /* accdigits>divdigits [cannot borrow] */
    }                 /* inner loop */
...
/* pointer (if any significant digits) */
    ...
    if (quodigits) {
      ...
      lsuq--;
      if (quodigits > DECPMAX + 1)
        break;          /* have enough */
    } else if (*lsuq) { /* first quotient digits */
      const uInt *pow;
      for (pow = DECPOWERS; *lsuq >= *pow; pow++)
        quodigits++;
      lsuq--;
    ...
    if (lsua > acc + DIVACCLEN - DIVOPLEN)
      continue;
    for (; msua > lsua && *msua == 0;)
      msua--;
    if (*msua == 0 && msua == lsua)
      break;
  } /* outer loop */
...
#define divsplit6 1000    /* divisor */
  ...
  if (op & DIVIDE)
    return decFinalize(result, &num, set); /* all done */
  ...
  if (num.exponent >= 0) { /* already an int, or need pad zeros */
    for (ub = num.lsd + 1; ub <= num.lsd + num.exponent; ub++)
      *ub = 0;
      ...
      *(num.msd - 1) = 0; /* in case of left carry, or make 0 */
      ...
      if (reround != 0) { /* discarding non-zero */
        ...
        if (bump != 0) {            /* need increment */
          ...
          ub = num.lsd;
          for (; UBTOUI(ub - 3) == 0x09090909; ub -= 4)
            UBFROMUI(ub - 3, 0);
          for (; *ub == 9; ub--)
            *ub = 0; /* at most 3 more */
          *ub += 1;
...
#if DECTRACE && 0
  printf("CoeffbL:");
  for (ui = bufl + MULOPLEN - 1; ui >= bufl; ui--)
    printf(" %08lx", (LI)*ui);
  printf("\n");
  printf("CoeffbR:");
  for (uj = bufr + MULOPLEN - 1; uj >= bufr; uj--)
    printf(" %08lx", (LI)*uj);
  printf("\n");
#endif
...
/* zero the accumulator */
  ...
  for (ui = bufr; ui < bufr + MULOPLEN; ui++) { /* over each item in rhs */
    if (*ui == 0)
      continue;              /* product cannot affect result */
    pl = accl + (ui - bufr); /* where to add the lhs */
    for (uj = bufl; uj < bufl + MULOPLEN;
         uj++, pl++) { /* over each item in lhs */
...
#error Unexpected type
  ...
  for (pl = accl, pa = acc; pl < accl + MULACCLEN;
       pl++, pa++) { /* each column position */
    uInt lo, hop;    /* work */
  ...
  for (pa = acc;; pa += 4) { /* zero the accumulator */
    ...
    *(pa + 3) = 0; /* [reduce overhead] */
    if (pa == acc + MULACCLEN * 2 - 4)
      break; /* multiple of 4 asserted */
  }          /* pa */
  ...
  for (ui = bufr;; ui++) {                  /* over each item in rhs */
    ...
    for (uj = bufl;; uj++, pa++) {          /* over each item in lhs */
      ...
      *(pa + MULACCLEN) += hi + (*pa < lo); /* .. high bits with any carry */
      if (uj == bufl + MULOPLEN - 1)
        break;
    }
    if (ui == bufr + MULOPLEN - 1)
      break;
  }
...
#error Unexpected type
    ...
    *(pa + 1) += estlo;
    ...
    if (*(pa + 1) < estlo)
      *(pa + 1 + MULACCLEN) += 1; /* carry */
    if (pa == acc + MULACCLEN - 2)
      break; /* [MULACCLEN-1 will never need split] */
  }          /* pa loop */
#endif
...
decFloat *decFloatAbs(decFloat *result, const decFloat *df, decContext *set) {
  if (DFISNAN(df))
    return decNaNs(result, df, NULL, set);
...
decFloat *decFloatAdd(decFloat *result, const decFloat *dfl,
...
#define CARRYPAT 0x00000001 /* carry=1 pattern */
  ...
  if (summ <= 8) {  /* MSD+MSD is good, or there is a special */
    if (summ < 0) { /* there is a special */
      ...
      if (summ < -64)
        return decNaNs(result, dfl, dfr, set); /* one or two NaNs */
      /* two infinities with different signs is invalid */
      if (summ == -64 && diffsign)
        return decInvalid(result, set);
      if (DFISINF(dfl))
        return decInfinity(result, dfl); /* LHS is infinite */
    ...
    if (bexpr == bexpl && !diffsign) {
      ...
      encode |=
          DECCOMBFROM[tac[DECLETS]]; /* add constructed combination field */
  ...
  if (overlap <= 0) { /* no overlap possible */
    ...
    if (overlap == DECPMAX) { /* aligned */
      ...
      if (buf + COFF + DECPMAX + 8 < ub + DECPMAX) {
        ...
        for (; us < ub + DECPMAX; us += 4, ut += 4)
          UBFROMUI(ut, UBTOUI(us));
      }
    } /* unaligned */
    ...
    for (; ut >= acc + 4; ut -= 4, us -= 4) { /* little-endian add loop */
      ...
      carry += UBTOUI(us); /* rhs + carry */
      if (carry == 0)
        continue;          /* no-op [common if unaligned] */
      carry += UBTOUI(ut); /* lhs */
      ...
      *ub += 1;
      ...
      if (ISCOEFFZERO(acc + COFF)) {
        ...
        if (ulsd > umsd) {               /* more to check */
          ...
          for (; UBTOUI(umsd) == 0 && umsd + 3 < ulsd;)
            umsd += 4;
          for (; *umsd == 0 && umsd < ulsd;)
            umsd++;
        }
...
/* ------------------------------------------------------------------ */
decFloat *decFloatAnd(decFloat *result, const decFloat *dfl,
                      const decFloat *dfr, decContext *set) {
...
#if DOUBLE
  DFWORD(result, 0) =
      ZEROWORD | ((DFWORD(dfl, 0) & DFWORD(dfr, 0)) & 0x04009124);
  DFWORD(result, 1) = (DFWORD(dfl, 1) & DFWORD(dfr, 1)) & 0x49124491;
#elif QUAD
  DFWORD(result, 0) =
      ZEROWORD | ((DFWORD(dfl, 0) & DFWORD(dfr, 0)) & 0x04000912);
...
/* ------------------------------------------------------------------ */
decFloat *decFloatCompare(decFloat *result, const decFloat *dfl,
                          const decFloat *dfr, decContext *set) {
  Int comp; /* work */
  /* NaNs are handled as usual */
  if (DFISNAN(dfl) || DFISNAN(dfr))
  ...
  if (comp < 0)
    DFBYTE(result, 0) |= 0x80; /* set sign bit */
  return result;
} /* decFloatCompare */
...
/*   returns result, which may be -1, 0, 1, or NaN (Unordered)	      */
/* ------------------------------------------------------------------ */
decFloat *decFloatCompareSignal(decFloat *result, const decFloat *dfl,
                                const decFloat *dfr, decContext *set) {
  Int comp; /* work */
...
/* ------------------------------------------------------------------ */
decFloat *decFloatCompareTotal(decFloat *result, const decFloat *dfl,
                               const decFloat *dfr) {
  Int comp;    /* work */
  uInt uiwork; /* for macros */
#if QUAD
...
/* (shift both right two if Quad to make a multiple of four) */
      ...
      comp = 0; /* assume equal */
      for (ub = bufl, uc = bufr; ub < bufl + DECPMAX + QUAD * 2;
           ub += 4, uc += 4) {
        uInt ui = UBTOUI(ub);
        if (ui == UBTOUI(uc))
          continue; /* so far so same */
...
/*   returns result, which may be -1, 0, or 1			      */
/* ------------------------------------------------------------------ */
decFloat *decFloatCompareTotalMag(decFloat *result, const decFloat *dfl,
                                  const decFloat *dfr) {
...
/* This is a bitwise operation; no errors or exceptions are possible. */
/* ------------------------------------------------------------------ */
decFloat *decFloatCopySign(decFloat *result, const decFloat *dfl,
                           const decFloat *dfr) {
...
uInt decFloatDigits(const decFloat *df) {
  ...
  if (!DFISNAN(df) && DECCOMBMSD[sourhi >> 26])
    return DECPMAX;
...
decFloat *decFloatDivide(decFloat *result, const decFloat *dfl,
                         const decFloat *dfr, decContext *set) {
...
/*								      */
/* ------------------------------------------------------------------ */
decFloat *decFloatDivideInteger(decFloat *result, const decFloat *dfl,
                                const decFloat *dfr, decContext *set) {
...
#define FMALEN (ROUNDUP4(1 + (DECPMAX9 * 18 + 1) + DECPMAX + 2))
  ...
  if (DFISSPECIAL(dfl) || DFISSPECIAL(dfr) || DFISSPECIAL(dff)) {
    ...
    if (DFISSNAN(dfl) || DFISSNAN(dfr))
      return decNaNs(result, dfl, dfr, set);
    if (DFISSNAN(dff))
      return decNaNs(result, dff, NULL, set);
    if (DFISNAN(dfl) || DFISNAN(dfr))
      return decNaNs(result, dfl, dfr, set);
    if (DFISNAN(dff))
      return decNaNs(result, dff, NULL, set);
  ...
  for (; UBTOUI(hi->msd) == 0 && hi->msd + 3 < hi->lsd;)
    hi->msd += 4;
  for (; *hi->msd == 0 && hi->msd < hi->lsd;)
    hi->msd++;
  for (; UBTOUI(lo->msd) == 0 && lo->msd + 3 < lo->lsd;)
    lo->msd += 4;
  for (; *lo->msd == 0 && lo->msd < lo->lsd;)
    lo->msd++;
  ...
  if (diffsign) {
    ...
    for (uh = hi->msd; uh < hi->lsd - 3; uh += 4)
      UBFROMUI(uh, 0x09090909 - UBTOUI(uh));
    for (; uh <= hi->lsd; uh++)
      *uh = (uByte)(0x09 - *uh);
  }
...
/* padding is still > 0, but will fit in acc (less leading carry slot) */
    ...
    for (; ul >= lo->msd && padding > 0; padding--, ul--, ub--)
      *ub = *ul;
    for (; padding > 0; padding--, ub--)
      *ub = 0; /* mind the gap */
  }
  ...
  if (!diffsign) { /* same-sign addition */
    ...
    if (!carry) { /* no carry out means hi<lo */
      ...
      for (ul = lo->msd; ul < lo->lsd - 3; ul += 4)
        UBFROMUI(ul, 0x09090909 - UBTOUI(ul));
      for (; ul <= lo->lsd; ul++)
        *ul = (uByte)(0x09 - *ul); /* [leaves ul at lsd+1] */
...
uInt decFloatIsCanonical(const decFloat *df) {
  if (DFISSPECIAL(df)) {
    if (DFISINF(df)) {
      if (DFWORD(df, 0) & ECONMASK)
        return 0; /* exponent continuation */
      if (!DFISCCZERO(df))
        return 0; /* coefficient continuation */
      return 1;
    }
    /* is a NaN */
    if (DFWORD(df, 0) & ECONNANMASK)
      return 0; /* exponent continuation */
    if (DFISCCZERO(df))
      return 1; /* coefficient continuation */
    /* drop through to check payload */
  }
  { /* declare block */
#if DOUBLE
...
/* ------------------------------------------------------------------ */
decFloat *decFloatMax(decFloat *result, const decFloat *dfl,
                      const decFloat *dfr, decContext *set) {
  Int comp;
  if (DFISNAN(dfl)) {
    /* sNaN or both NaNs leads to normal NaN processing */
    if (DFISNAN(dfr) || DFISSNAN(dfl))
      return decNaNs(result, dfl, dfr, set);
...
/* ------------------------------------------------------------------ */
decFloat *decFloatMaxMag(decFloat *result, const decFloat *dfl,
                         const decFloat *dfr, decContext *set) {
  Int comp;
  decFloat absl, absr;
  if (DFISNAN(dfl) || DFISNAN(dfr))
...
/* If just one operand is a quiet NaN it is ignored.		      */
/* ------------------------------------------------------------------ */
decFloat *decFloatMin(decFloat *result, const decFloat *dfl,
                      const decFloat *dfr, decContext *set) {
  Int comp;
  if (DFISNAN(dfl)) {
    /* sNaN or both NaNs leads to normal NaN processing */
...
/* ------------------------------------------------------------------ */
decFloat *decFloatMinMag(decFloat *result, const decFloat *dfl,
                         const decFloat *dfr, decContext *set) {
  Int comp;
  decFloat absl, absr;
  if (DFISNAN(dfl) || DFISNAN(dfr))
...
/*								      */
/* ------------------------------------------------------------------ */
decFloat *decFloatMultiply(decFloat *result, const decFloat *dfl,
                           const decFloat *dfr, decContext *set) {
  bcdnum num;                      /* for final conversion */
...
/* number.							      */
/* ------------------------------------------------------------------ */
decFloat *decFloatNextToward(decFloat *result, const decFloat *dfl,
                             const decFloat *dfr, decContext *set) {
  ...
  comp = decNumCompare(dfl, dfr, 0);
  if (comp == 0)
    return decFloatCopySign(result, dfl, dfr); /* equal */
...
/* comprise just zeros and ones; if not, Invalid operation results.   */
/* ------------------------------------------------------------------ */
decFloat *decFloatOr(decFloat *result, const decFloat *dfl, const decFloat *dfr,
                     decContext *set) {
  if (!DFISUINT01(dfl) || !DFISUINT01(dfr) || !DFISCC01(dfl) || !DFISCC01(dfr))
    return decInvalid(result, set);
/* the operands are positive finite integers (q=0) with just 0s and 1s */
...
/* ------------------------------------------------------------------ */
decFloat *decFloatQuantize(decFloat *result, const decFloat *dfl,
                           const decFloat *dfr, decContext *set) {
  ...
  uByte
      buf[4 + DECPMAX * 3 + 2 * QUAD]; /* + space for zeros to left or right */
#if DECTRACE
  ...
  if (drop == 0)
    return decCanonical(result, dfl); /* return canonical */
...
#define BUFOFF (buf + 4 + DECPMAX)
  ...
  if (drop > 0) { /* [most common case] */
    ...
    if (reround != 0) { /* discarding non-zero */
      ...
      if (set->round == DEC_ROUND_HALF_EVEN) { /* fastpath slowest case */
        ...
        case DEC_ROUND_CEILING: {
          ...
          if (!(sourhil & DECFLOAT_Sign) && reround > 0)
            bump = 1;
          break;
        } /* r-c */
    ...
    if (drop > 4) {
      ...
      for (uc = BUFOFF - 12; uc > ulsd - DECPMAX - 3; uc -= 4)
        UBFROMUI(uc, 0);
    }
  } /* need round (drop>0) */
...
decFloat *decFloatReduce(decFloat *result, const decFloat *df,
  ...
  if (df != result)
    *result = *df; /* copy, if needed */
  if (DFISNAN(df))
    return decNaNs(result, df, NULL, set); /* sNaN */
  /* zeros and infinites propagate too */
  if (DFISINF(df))
    return decInfinity(result, df); /* canonical */
  if (DFISZERO(df)) {
  ...
  if (*ub)
    return result; /* no trailing zeros */
  for (ub--; *ub == 0;)
    ub--; /* terminates because non-zero */
  /* *ub is the first non-zero from the right */
  num.sign = DFWORD(df, 0) & DECFLOAT_Sign; /* set up number... */
  num.exponent =
      GETEXPUN(df) + (Int)(buf + DECPMAX - 1 - ub); /* adjusted exponent */
  num.msd = buf;
  num.lsd = ub;
...
/*								      */
/* ------------------------------------------------------------------ */
decFloat *decFloatRemainder(decFloat *result, const decFloat *dfl,
                            const decFloat *dfr, decContext *set) {
...
/* ------------------------------------------------------------------ */
decFloat *decFloatRemainderNear(decFloat *result, const decFloat *dfl,
                                const decFloat *dfr, decContext *set) {
...
decFloat *decFloatRotate(decFloat *result, const decFloat *dfl,
                         const decFloat *dfr, decContext *set) {
  ...
  if (digits > 2)
    return decInvalid(result, set); /* definitely out of range */
  rotate = DPD2BIN[DFWORD(dfr, DECWORDS - 1) & 0x3ff]; /* is in bottom declet */
  if (rotate > DECPMAX)
    return decInvalid(result, set); /* too big */
  /* [from here on no error or status change is possible] */
  if (DFISINF(dfl))
    return decInfinity(result, dfl); /* canonical */
  /* handle no-rotate cases */
  if (rotate == 0 || rotate == DECPMAX)
    return decCanonical(result, dfl);
  ...
  ub = buf;
  if (rotate < 0)
    ub += PHALF; /* rotate right, so space to left */
  GETCOEFF(dfl, ub);
...
uInt decFloatSameQuantum(const decFloat *dfl, const decFloat *dfr) {
  ...
  if (GETEXP(dfl) == GETEXP(dfr))
    return 1; /* biased exponents match */
  return 0;
} /* decFloatSameQuantum */
...
decFloat *decFloatScaleB(decFloat *result, const decFloat *dfl,
                         const decFloat *dfr, decContext *set) {
  uInt digits; /* work */
...
#endif
  if (expr > SCALEBMAX)
    return decInvalid(result, set); /* oops */
...
/* operand is an sNaN.	The result is canonical.		      */
/* ------------------------------------------------------------------ */
decFloat *decFloatShift(decFloat *result, const decFloat *dfl,
                        const decFloat *dfr, decContext *set) {
  ...
  if (DFISSIGNED(dfr)) { /* shift right */
    ...
    if (shift > 8)
      memset(buf + DECPMAX + 8, 0, 8 + QUAD * 18); /* all other cases */
    num.msd += shift;
    num.lsd = num.msd + DECPMAX - 1;
  }
  decFinalize(result, &num, set);
...
/*								      */
/* ------------------------------------------------------------------ */
decFloat *decFloatSubtract(decFloat *result, const decFloat *dfl,
                           const decFloat *dfr, decContext *set) {
  decFloat temp;
  /* NaNs must propagate without sign change */
  if (DFISNAN(dfr))
    return decFloatAdd(result, dfl, dfr, set);
...
/* comprise just zeros and ones; if not, Invalid operation results.   */
/* ------------------------------------------------------------------ */
decFloat *decFloatXor(decFloat *result, const decFloat *dfl,
                      const decFloat *dfr, decContext *set) {
  if (!DFISUINT01(dfl) || !DFISUINT01(dfr) || !DFISCC01(dfl) || !DFISCC01(dfr))
    return decInvalid(result, set);
/* the operands are positive finite integers (q=0) with just 0s and 1s */
...
/* ------------------------------------------------------------------ */
static decFloat *decNaNs(decFloat *result, const decFloat *dfl,
                         const decFloat *dfr, decContext *set) {
  /* handle sNaNs first */
  if (dfr != NULL && DFISSNAN(dfr) && !DFISSNAN(dfl))
    dfl = dfr; /* use RHS */
  if (DFISSNAN(dfl)) {
  ...
  if (!DFISNAN(dfl))
    dfl = dfr;                      /* RHS must be NaN, use it */
...
static Int decNumCompare(const decFloat *dfl, const decFloat *dfr, Flag tot) {
  ...
  if (DFISSIGNED(dfl)) {
    if (!DFISSIGNED(dfr)) { /* -LHS +RHS */
      if (DFISZERO(dfl) && DFISZERO(dfr) && !tot)
        return 0;
      return -1; /* RHS wins */
    }
    sigl = -1;
  ...
  if (DFISINF(dfl)) {
    if (DFISINF(dfr))
      return 0;  /* both infinite & same sign */
    return sigl; /* inf > n */
  }
  if (DFISINF(dfr))
    return sigr; /* n < inf [dfl is finite] */
...
/* (shift both right two if Quad to make a multiple of four) */
  ...
  if (shift == 0) {               /* aligned; common and easy */
    ...
    if (shift > 8) {
      ...
      for (up = bufl + DECPMAX + QUAD * 2 + 8; up < upend; up += 4)
        UBFROMUI(up, 0);
      /* [pads up to 36 in all for Quad] */
      for (;; ub += 4) {
        if (UBTOUI(ub) != 0)
          return sigl;
        if (ub + 4 > bufl + shift - 4)
          break;
      }
    }
    ...
    if (shift < -8) {
      ...
      for (up = bufr + DECPMAX + QUAD * 2 + 8; up < upend; up += 4)
        UBFROMUI(up, 0);
      /* [pads up to 36 in all for Quad] */
      for (;; uc += 4) {
        if (UBTOUI(uc) != 0)
          return sigr;
        if (uc + 4 > bufr - shift - 4)
          break;
      }
    }
    /* check remaining leading digits */
  ...
  if (!tot)
    return 0; /* numerically equal */
  /* total ordering .. exponent matters */
  if (shift > 0)
    return sigl; /* total order by exponent */
  if (shift < 0)
    return sigr; /* .. */
  return 0;
} /* decNumCompare */
...
/* ------------------------------------------------------------------ */
static uInt decToInt32(const decFloat *df, decContext *set, enum rounding rmode,
