...
ytp_iterator_t ytp_data_commit(ytp_yamal_t *yamal, int64_t ts, void *data, fmc_error_t **error) {
...
void ytp_data_read(ytp_yamal_t *yamal, ytp_iterator_t iterator, uint64_t *seqno, int64_t *ts, ytp_mmnode_offs *stream, size_t *sz, const char **data, fmc_error_t **error) {
  ...
  ytp_time_read(yamal, iterator, seqno, ts, &read_sz, (const char **)&msg, error);
  if (*error) {
    return;
  }
