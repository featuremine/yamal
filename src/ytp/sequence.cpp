...
ytp_sequence_t *ytp_sequence_new_2(fmc_fd fd, bool enable_thread,
                                   fmc_error_t **error) {
  auto *sequence = static_cast<ytp_sequence_t *>(aligned_alloc(alignof(ytp_sequence_t), sizeof(ytp_sequence_t)));
  if (!sequence) {
...
ytp_sequence::ytp_sequence(fmc_fd fd, bool enable_thread) : ctrl(fd, enable_thread), timeline(&ctrl) {}
...
ytp_sequence_shared::ytp_sequence_shared(const char *filename, fmc_fmode mode) : file(filename, mode), seq(file.value, true) {}

ytp_sequence_shared_t *ytp_sequence_shared_new(const char *filename,
                                               fmc_fmode mode,
                                               fmc_error_t **error) {
  try {
    auto *seq = static_cast<ytp_sequence_shared *>(aligned_alloc(alignof(ytp_sequence_shared), sizeof(ytp_sequence_shared)));
