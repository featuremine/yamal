...
ytp_timeline::ytp_timeline(ytp_control_t *ctrl) : ctrl(ctrl) {
  ...
  ytp_timeline_indx_cb(this, YTP_CHANNEL_ANN, channel_announcement_msg, this,
                       &error);
  if (error) {
    throw fmc::error(*error);
  }
...
bool ytp_timeline_term(ytp_timeline_t *timeline) {
  return ytp_yamal_term(timeline->read) &&
         ytp_cursor_term_ann(&timeline->ctrl->data_cursor) &&
         timeline->ctrl->poll_result.state == poll_result_t::state_t::NONE;
}
