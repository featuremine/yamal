...
static bool ytp_timeline_poll_ann(ytp_timeline_t *timeline,
  ...
  ytp_announcement_read(&timeline->ctrl->yamal, timeline->it_ann, &seqno, &psz,
  ...
  if (!was_announced(timeline->ch_announced, channelid - YTP_CHANNEL_OFF)) {
    timeline->cb_ch.lock();
  ...
  auto next =
      ytp_announcement_next(&timeline->ctrl->yamal, timeline->it_ann, error);
  if (*error) {
    return false;
  }

  timeline->it_ann = next;
  return true;
}
