...
parse_ann_payload(const char *data, std::size_t sz, fmc_error_t **error) {
  ...
  auto encoding =
      std::string_view(hdr.payload + peer_name_sz + channel_name_sz,
                       sz - channel_name_sz - peer_name_sz - sizeof(ann_msg_t));

  return {peername, chname, encoding, &hdr.subscription};
}

ytp_cursor_t *ytp_cursor_new(ytp_yamal_t *yamal, fmc_error_t **error) {
  auto *cursor = static_cast<ytp_cursor_t *>(
      aligned_alloc(alignof(ytp_cursor_t), sizeof(ytp_cursor_t)));
  if (!cursor) {
...
ytp_cursor::ytp_cursor(ytp_yamal_t *yamal)
    : yamal(yamal), ann_processed(0), data_processed(0) {
  fmc_error_t *error;
...
void ytp_cursor_ann_cb_rm(ytp_cursor_t *cursor, ytp_cursor_ann_cb_t cb,
  ...
  std::erase_if(l,
                [&](const ytp_cursor_ann_cb_cl_t &item) { return c == item; });
}

void ytp_cursor_data_cb(ytp_cursor_t *cursor, ytp_stream_t stream,
  ...
  if (!cursor->yamal->readonly_) {
    ...
    ytp_yamal_read(cursor->yamal, stream_it, &stream_seqno, &stream_sz,
                   &stream_dataptr, error);
    if (*error) {
      return;
    }

    auto &const_sub =
        *std::get<3>(parse_ann_payload(stream_dataptr, stream_sz, error));
    uint64_t unset = SUBSCRIPTION_STATE::YTP_SUB_NO_SUBSCRIPTION;
...
void ytp_cursor_data_cb_rm(ytp_cursor_t *cursor, ytp_stream_t stream,
                           ytp_cursor_data_cb_t cb, void *closure,
                           fmc_error_t **error) {
  fmc_error_clear(error);
  if (auto where = cursor->cb_data.find(stream);
      where != cursor->cb_data.end()) {
    auto &l = where->second;
    auto c = ytp_cursor_data_cb_cl_t(cb, closure);
    std::erase_if(
        l, [&](const ytp_cursor_data_cb_cl_t &item) { return c == item; });
  }
}
...
bool ytp_cursor_term_ann(ytp_cursor_t *cursor) {
  ...
  size_t stream_sz;
  const char *stream_dataptr;
  ytp_yamal_read(cursor->yamal, cursor->it_ann, &stream_seqno, &stream_sz,
                 &stream_dataptr, &error);
  if (error) {
    return true;
  }
  ...
  auto sub =
      std::get<3>(parse_ann_payload(stream_dataptr, stream_sz, &error))->load();
  return sub < SUBSCRIPTION_STATE::YTP_SUB_NO_SUBSCRIPTION;
}
...
static std::tuple<ytp_stream_t, std::string_view>
parse_data_payload(const char *data, std::size_t sz, fmc_error_t **error) {
...
bool ytp_cursor_poll_ann(ytp_cursor_t *cursor, fmc_error_t **error) {
  ...
  if (sub == SUBSCRIPTION_STATE::YTP_SUB_UNKNOWN) {
    return false;
  }
  ...
  if (sub == SUBSCRIPTION_STATE::YTP_SUB_DUPLICATED) {
    return true;
  }
  ...
  for (auto it = cursor->cb_ann.begin(); it != cursor->cb_ann.end(); ++it) {
    ...
    auto &cb = *it;
    cb.first(cb.second, stream, seqno, peername.size(), peername.data(),
             chname.size(), chname.data(), encoding.size(), encoding.data());
  }
...
extern bool ytp_cursor_poll_data(ytp_cursor_t *cursor, fmc_error_t **error) {
  ...
  ytp_time_read(cursor->yamal, cursor->it_data, &seqno, &msgtime, &sz, &dataptr,
                error);
  if (*error) {
    return false;
  }
...
template <typename T>
static void concat_callbacks(fmc::lazy_rem_vector<T> &d,
                             fmc::lazy_rem_vector<T> &s) {
...
void ytp_cursor_seek(ytp_cursor_t *cursor, uint64_t offset,
                     fmc_error_t **error) {
...
ytp_anns_t *ytp_anns_new(ytp_yamal_t *yamal, fmc_error_t **error) {
  auto *anns = static_cast<ytp_anns_t *>(
      aligned_alloc(alignof(ytp_anns_t), sizeof(ytp_anns_t)));
  if (!anns) {
    fmc_error_set2(error, FMC_ERROR_MEMORY);
    return {};
  }
...
ytp_stream_t ytp_anns_stream(ytp_anns_t *anns, size_t pz, const char *pn,
                             size_t cz, const char *cn, size_t ez,
                             const char *en, fmc_error_t **error) {
  fmc_error_clear(error);
  ...
  if (auto it = anns->reverse_map.find({arg_peer, arg_ch});
      it != anns->reverse_map.end()) {
    ...
    size_t stream_sz;
    const char *stream_dataptr;
    ytp_yamal_read(anns->yamal, it_stream, &stream_seqno, &stream_sz,
                   &stream_dataptr, error);
    if (*error) {
      return {};
    }
    ...
    auto [peername, chname, encoding, const_sub] =
        parse_ann_payload(stream_dataptr, stream_sz, error);
    if (*error) {
      return {};
    }
  ...
  ytp_anns_lookup_one(
      anns, error,
      [&](ytp_stream_t stream, std::string_view peer, std::string_view ch,
...
static void ytp_any_next(ytp_yamal_t *yamal, ytp_iterator_t &it, size_t *sz,
...
bool ytp_inds_next(ytp_inds_t *inds, ytp_stream_t *id, uint64_t *offset,
  ...
  ytp_any_next(inds->yamal, inds->it_idx, sz, data, error,
               [&](fmc_error_t **error) {
...
void ytp_subs_init(ytp_subs_t *subs, ytp_yamal_t *yamal, fmc_error_t **error) {
  if (yamal->readonly_) {
    fmc_error_set(error,
                  "unable to initialize using a readonly file descriptor");
    return;
  }
...
bool ytp_subs_next(ytp_subs_t *subs, ytp_stream_t *id, fmc_error_t **error) {
  ...
  while (!ytp_yamal_term(subs->it_subs)) {
    ytp_any_next(subs->yamal, subs->it_subs, &sz, &data, error,
                 [&](fmc_error_t **error) {
                   ...
                   *id = ye64toh(submsg.stream);
                   ...
                   auto off_sub =
                       ytp_yamal_tell(subs->yamal, subs->it_subs, error);
                   if (*error) {
                     return;
                   }
                   ...
                   size_t stream_sz;
                   const char *stream_dataptr;
                   ytp_yamal_read(subs->yamal, it_stream, &stream_seqno,
                                  &stream_sz, &stream_dataptr, error);
                   if (*error) {
                     return;
                   }
                   ...
                   uint64_t prev_val =
                       SUBSCRIPTION_STATE::YTP_SUB_NO_SUBSCRIPTION;
                   auto &sub = const_cast<std::atomic<uint64_t> &>(*const_sub);
                   ret = sub.compare_exchange_weak(prev_val, off_sub) ||
                         prev_val == off_sub;
                 });
    if (*error || ret) {
      return !*error;
...
char *ytp_stream_reserve(ytp_yamal_t *yamal, size_t sz, fmc_error_t **error) {
  fmc_error_clear(error);
  auto *data_msg =
      (data_msg_t *)ytp_time_reserve(yamal, sz + sizeof(data_msg_t), error);
  if (*error) {
    return nullptr;
  }
...
ytp_iterator_t ytp_stream_commit(ytp_yamal_t *yamal, uint64_t msgtime,
                                 ytp_stream_t id, void *data,
                                 fmc_error_t **error) {
  auto *stream_msg = (data_msg_t *)((char *)data - sizeof(data_msg_t));
  stream_msg->stream = htoye64(id);
  return ytp_time_commit(yamal, msgtime, stream_msg, YTP_STREAM_LIST_DATA,
                         error);
}

void ytp_stream_read_ann(ytp_yamal_t *yamal, ytp_stream_t id, uint64_t *seqno,
                         size_t *pz, const char **pn, size_t *cz,
                         const char **cn, size_t *ez, const char **en,
                         uint64_t *sub, fmc_error_t **error) {
...
ytp_iterator_t ytp_stream_write_idx(ytp_yamal_t *yamal, ytp_stream_t id,
                                    uint64_t offset, size_t sz,
                                    const char *data, fmc_error_t **error) {
  auto *ptr =
      (idx_msg_t *)ytp_yamal_reserve(yamal, sz + sizeof(idx_msg_t), error);
  if (*error) {
    return {};
  }
  ptr->stream = id;
...
ytp_iterator_t ytp_stream_write_ann(ytp_yamal_t *yamal, size_t pz,
                                    const char *pn, size_t cz, const char *cn,
                                    size_t ez, const char *en,
                                    fmc_error_t **error) {
  ann_msg_t *ptr;
  constexpr auto max_pz =
      (size_t)std::numeric_limits<decltype(ptr->peer_name_sz)>::max();
  constexpr auto max_cz =
      (size_t)std::numeric_limits<decltype(ptr->channel_name_sz)>::max();
  if (pz > max_pz) {
  ...
  ptr = (ann_msg_t *)ytp_yamal_reserve(yamal, pz + cz + ez + sizeof(ann_msg_t),
                                       error);
  if (*error) {
    return {};
  }
  ...
  ptr->subscription = SUBSCRIPTION_STATE::YTP_SUB_UNKNOWN;
...
ytp_iterator_t ytp_stream_write_sub(ytp_yamal_t *yamal, ytp_stream_t id,
                                    fmc_error_t **error) {
