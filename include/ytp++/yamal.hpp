    ...
    public:
        ...
        data::base_iterator<forward>& operator++() {
            ...
            fmc_runtime_error_unless(!err)
                << "unable to obtain next position of iterator with error :" << fmc_error_msg(err);
            return *this;
        }
        data::base_iterator<forward>& operator--() {
            fmc_error_t *err = nullptr;
            ...
            fmc_runtime_error_unless(!err)
                << "unable to obtain previous position of iterator with error :" << fmc_error_msg(err);
            return *this;
        }
        uint64_t operator uint64_t() {
            ...
            fmc_runtime_error_unless(!err)
                << "unable to tell position of iterator with error :" << fmc_error_msg(err);
            return off;
        }
        ...
        std::tuple<uint64_t, int64_t, stream, const std::string_view> operator*() {
            ...
            std::make_tuple<uint64_t, int64_t, stream, const std::string_view>(seqno, ts, stream(sid), std::string_view(msgdata, msgsz))
        }
    private:
...
class streams {
public:
    stream announce(std::string_view channel, std::string_view peer, std::string_view encoding) {
        fmc_error_t *err = nullptr;
        ytp_iterator_t sid = ytp_announcement_write(yamal_, peer.size(), peer.data(),
...
public:
    yamal(std::string_view fname, bool readonly) {
        ...
        fd_ = fmc_fopen(fname.data(),
                        fmc_fmode::READ | fmc_fmode((!readonly) * fmc_fmode::WRITE)
                        &err);
        fmc_runtime_error_unless(!err)
