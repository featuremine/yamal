...
enum SUBSCRIPTION_STATE {
  YTP_SUB_UNKNOWN = 0,
  YTP_SUB_DUPLICATED = 1,
  YTP_SUB_NO_SUBSCRIPTION = 2,
};
 ...
 * @param[out] error out-parameter for error handling
 */
FMMODFUNC void ytp_cursor_data_cb(ytp_cursor_t *cursor, ytp_stream_t stream,
 ...
 * @param[out] error out-parameter for error handling
 */
FMMODFUNC void ytp_cursor_data_cb_rm(ytp_cursor_t *cursor, ytp_stream_t stream,
                                     ytp_cursor_data_cb_t cb, void *closure,
                                     fmc_error_t **error);
 ...
 * @return serializable
 */
FMMODFUNC uint64_t ytp_cursor_tell(ytp_cursor_t *cursor, fmc_error_t **error);

/**
 * @brief Allocates and initializes a ytp_anns object
 *
 * yamal needs to be writable since announcementscription object is used by
 * publishers only
 * @param[in] cursor the ytp_cursor_t object
 * @param[out] error out-parameter for error handling
 * @return ytp_anns_t object
 */
FMMODFUNC ytp_anns_t *ytp_anns_new(ytp_yamal_t *yamal, fmc_error_t **error);
 ...
 * @param[out] error out-parameter for error handling
 */
FMMODFUNC void ytp_anns_destroy(ytp_anns_t *cursor, fmc_error_t **error);
 ...
 * @param[out] error out-parameter for error handling
 */
FMMODFUNC ytp_stream_t ytp_anns_stream(ytp_anns_t *cursor, size_t pz,
                                       const char *pn, size_t cz,
                                       const char *cn, size_t ez,
 ...
 *
 * Enforces protocol by setting stream announcement indscription flag is not
 * set. Skips duplicate indscription messages.
...
FMMODFUNC bool ytp_inds_next(ytp_inds_t *cursor, ytp_stream_t *id,
                             uint64_t *offset, size_t *sz, const char **data,
                             fmc_error_t **error);
 ...
 *
 * Enforces protocol by setting stream announcement subscription flag is not
 * set. Skips duplicate subscription messages.
 * @param[in] subs the ytp_subs_t object
 * @param[out] id of the stream
 * @param[out] error out-parameter for error handling
 * @return true if advanced, false if at the end of the list
 */
FMMODFUNC bool ytp_subs_next(ytp_subs_t *subs, ytp_stream_t *id,
                             fmc_error_t **error);
 ...
 */
FMMODFUNC char *ytp_stream_reserve(ytp_yamal_t *yamal, size_t sz,
                                   fmc_error_t **error);
 ...
 * @param[out] error out-parameter for error handling
 */
FMMODFUNC void ytp_stream_read_ann(ytp_yamal_t *yamal, ytp_stream_t id,
                                   uint64_t *seqno, size_t *pz, const char **pn,
                                   size_t *cz, const char **cn, size_t *ez,
                                   const char **en, uint64_t *sub,
                                   fmc_error_t **error);
 ...
 * @return ytp_iterator_t for the message
 */
FMMODFUNC ytp_iterator_t ytp_stream_write_idx(ytp_yamal_t *yamal,
                                              ytp_stream_t id, uint64_t offset,
                                              size_t sz, const char *data,
                                              fmc_error_t **error);
 ...
 * @return ytp_iterator_t for the message
 */
FMMODFUNC ytp_iterator_t ytp_stream_write_ann(ytp_yamal_t *yamal, size_t pz,
                                              const char *pn, size_t cz,
                                              const char *cn, size_t ez,
                                              const char *en,
                                              fmc_error_t **error);
 ...
 * @return ytp_iterator_t for the message
 */
FMMODFUNC ytp_iterator_t ytp_stream_write_sub(ytp_yamal_t *yamal,
                                              ytp_stream_t id,
                                              fmc_error_t **error);
